...
const categories = ["All Collections", "Verified Brands", "Verified Artists", "New Drops", "Live Shows"];
...
const ProductPage: React.FC = () => {
  ...
  const fetchProducts = async (category: string) => {
    ...
    const categoryQuery = validCategory ? `?category=${encodeURIComponent(validCategory)}` : "";

    try {
      const response = await axios.get(`${server}/api/product/productsbycategory${categoryQuery}`);
        ...
        {categories.map((category) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm transition ${
              activeCategory === category ? "bg-purple-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"
            }`}
          >
            {category}
          </button>
        ...
        {products.map((product) => (
          <div key={product._id} className="bg-gray-900 p-4 rounded-lg shadow-lg">
            ...
            <div className="mt-4">
              ...
              <p className="text-purple-400 font-bold mt-2">Rs. {product.price}</p>
      ...
      {showLoginDialog && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg">
            <p className="text-white text-lg mb-4">Please log in to add products to the cart.</p>
